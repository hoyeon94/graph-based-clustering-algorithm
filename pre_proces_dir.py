#meta data file을 directed 그래프로 전처리를 하기 위한 코드
import string
import re
...
#그래프 생성
copurchased_dir_graph = networkx.DiGraph()
for asin,data in musics.items():
    copurchased_dir_graph.add_node(asin)
    for target in data['copurchased'].split():
        copurchased_dir_graph.add_node(target.strip())
        ...
        copurchased_dir_graph.add_weighted_edges_from([(asin, target.strip(), cal_weight)])
...
writefile = open('musics-copurchase-dir.edgelist', 'wb')
networkx.write_weighted_edgelist(copurchased_dir_graph, writefile)
writefile.close()